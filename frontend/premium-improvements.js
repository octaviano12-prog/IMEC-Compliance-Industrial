(function () {
  ...
  function renderExecutiveBlock() {
    ...
    return '<section class="premium-header premium-panel suite-no-print"><div><h3>Centro executivo de conformidade</h3><p>Leitura rapida para decidir renovacoes, cobrancas e prioridades do dia.</p></div><div class="suite-action-group"><button class="btn btn-outline btn-sm" onclick="openPremiumActionPlan()">Plano de acao</button><button class="btn btn-primary btn-sm" onclick="navigate(\'reports\')">Relatorios</button></div></section>' +
      '<section class="premium-grid suite-no-print">' +
        '<div class="premium-card premium-panel"><div class="premium-title"><span>Saude operacional</span><span class="premium-chip ' + (score >= 85 ? 'green' : score >= 65 ? 'orange' : 'red') + '">' + score + '%</span></div><div class="premium-progress"><span style="width:' + score + '%"></span></div><div class="premium-kpi mt-4"><div><span>Vencidos</span><strong>' + expired + '</strong></div><div><span>7 dias</span><strong>' + next7 + '</strong></div><div><span>Ativos</span><strong>' + activeEmployees().length + '</strong></div></div></div>' +
        '<div class="premium-card wide premium-panel"><div class="premium-title"><span>Emissao de certificados</span><span class="text-xs text-slate-400">12 meses</span></div>' + monthlyBars() + '</div>' +
