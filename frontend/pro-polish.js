(function () {
  ...
  function cardHtml(emp) {
    ...
    return '<article class="nr-id-card" id="nr-card-' + emp.id + '">'
      ...
      + '<div class="nr-actions"><label class="btn btn-outline btn-sm"><input class="nr-photo-input" type="file" accept="image/*" onchange="saveEmployeeCardPhoto(\'' + emp.id + '\', this)">Adicionar foto</label><button class="btn btn-primary btn-sm" onclick="printEmployeeCard(\'' + emp.id + '\')">Imprimir / PDF</button></div>';
  }

  function installIdCards() {
    if (typeof renderers === 'undefined') return false;
    renderers.idcards = async function () {
      var db = getDB();
      var employees = (db.employees || []).filter(function (emp) { return emp.status === 'ativo'; });
      var body = employees.length ? employees.map(cardHtml).join('') : '<div class="pro-empty">' + icon('file') + '<div><strong>Nenhum funcionario ativo</strong><p class="mt-1 text-sm">Cadastre um funcionario ativo para gerar a carteirinha.</p></div></div>';
